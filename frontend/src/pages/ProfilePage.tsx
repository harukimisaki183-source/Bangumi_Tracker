...
import { Camera, Save, LogOut, Film, FileText, KeyRound } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-6 w-fit">
          <button onClick={() => setTab("works")} className={tabClass("works")}><Film className="w-4 h-4" /> 我的番剧</button>
          <button onClick={() => setTab("posts")} className={tabClass("posts")}><FileText className="w-4 h-4" /> 我的帖子</button>
        </div>
        ...
        {tab === "works" ? (
          ...
          ) : <div className="text-center py-16 text-gray-400">还没有作品，<Link to="/works/create" className="text-indigo-600 hover:underline">去创建</Link></div>
        ) : (
          posts.length > 0 ? (
          ...
          ) : <div className="text-center py-16 text-gray-400">还没有帖子，<Link to="/community" className="text-indigo-600 hover:underline">去发布</Link></div>
        )}
      </motion.div>
    </div>
  );
